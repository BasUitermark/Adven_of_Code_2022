...
{
	if (inRange(range[0], range[1], range[2]) && inRange(range[0], range[1], range[3]))
		return (1);
	else if (inRange(range[2], range[3], range[0]) && inRange(range[2], range[3], range[1]))
		return (1);
	return (0);
}
...
bool	executePart1(std::vector<std::string> inputArray)
{
	...
	std::cout << GREEN BOLD "Total pair overlap: " << score << RESET << std::endl;
	return (true);
}